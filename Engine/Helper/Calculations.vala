    ...
    public class Calculations
    {
        ...
        public static float get_collision_distance(Ray ray, Vec3 size, Mat4 model_matrix)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                // Ray parallel to this slab. Standard slab-test handling: if
                // the ray's origin lies between the slab planes the slab
                // imposes no constraint; otherwise the ray misses entirely.
                // (Bailing out unconditionally here used to make picking fail
                // whenever the ray was perpendicular to an OBB axis — which
                // happens for a head-on camera looking at the tile's
                // centerline.)
                if (Math.fabsf(f) < 0.001f)
                {
                    if (-e - size[i] * l > 0 || -e + size[i] * l < 0)
                        return -1;
                    continue;
                }
