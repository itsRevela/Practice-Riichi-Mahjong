...
public class ClientRoundState : Object
{
    ...
    {
        Environment.log(LogType.INFO, "ClientRoundState", "Turn decision, self_active=" + self_active.to_string());
        if (self_active)
            JsonStateExport.export_turn_decision(state);
        do_turn_decision();
    }
    ...
    public void server_call_decision(ServerMessage message)
    {
        ...
        if (self_active)
            JsonStateExport.export_call_decision(state, tile, player);
